// tracking.js — ProgressTrackingViz component (Babel standalone, global vars)
// Depends on: config.js (STATUS_CFG, ETA_CFG, PO_COLS, GANTT_COLORS, COL_W)

const { useState: useStateTracking, useMemo: useMemoTracking } = React;
...
function DailyReportPanel() {
  const [filter, setFilter]               = useStateTracking("all");
  const [machineFilter, setMachineFilter] = useStateTracking("");
  const [search, setSearch]               = useStateTracking("");
  const [subTab, setSubTab]               = useStateTracking("table");
  const [selectedRow, setSelectedRow]     = useStateTracking(null);
  const [showIdle, setShowIdle]           = useStateTracking(true);

  const FLAT_DATA = useMemoTracking(() => {
    const rows = [];
  ...
  const filtered = useMemoTracking(() => {
            ...
            <table style={{ borderCollapse:"collapse", width:"100%", fontSize:10.5 }}>
              ...
              <tbody>
                {filtered.map((row, i) => {
                  ...
                  const isSubRow = !row._isFirstJob;
                  const isLastJobOfMachine = row._machineJobIndex === row._machineTotalJobs - 1;
                  return (
                    <tr key={row.lastestMachineNo + row.poNo + i}
                      className={"row-hover" + (isSel ? " row-sel" : "") + (row.idle ? " row-idle" : "")}
                      onClick={() => setSelectedRow(isSel ? null : i)}
                      style={{
                        borderBottom: isLastJobOfMachine ? "2px solid rgba(26,36,56,0.9)" : "1px solid rgba(59,130,246,0.12)",
                        cursor: row.idle ? "default" : "pointer",
                        background: isSel ? "rgba(59,130,246,0.12)" : isSubRow ? "rgba(14,20,36,0.6)" : "transparent",
                      }}>
                      <td style={{ padding:"3px 5px", fontWeight:600, color:row.idle?"#2a3a55":isSubRow?"transparent":"#3b82f6", borderLeft:isSubRow?"3px solid rgba(59,130,246,0.25)":"3px solid transparent", position:"relative" }}>
                        {!isSubRow && row.lastestMachineNo}
                        {!isSubRow && row._machineTotalJobs > 1 && (
                          <span style={{ display:"inline-block", marginLeft:3, padding:"0px 4px", borderRadius:2, background:"rgba(59,130,246,0.18)", border:"1px solid rgba(59,130,246,0.3)", color:"#60a5fa", fontSize:7.5, fontWeight:700, verticalAlign:"middle" }}>
                    ...
                    return (
                      <tr key={row.lastestMachineNo + ri} className="row-hover"
                        ...
                        {GANTT_DATES.map((d, di) => {
                          ...
                          return (
                            <td key={di} style={{
                              position:"relative", width:COL_W,
                              background:isToday?"rgba(245,158,11,0.05)":isWE?"repeating-linear-gradient(45deg,rgba(26,36,56,0.3),rgba(26,36,56,0.3) 2px,transparent 2px,transparent 6px)":row.idle?"repeating-linear-gradient(135deg,rgba(20,28,44,0.4),rgba(20,28,44,0.4) 2px,transparent 2px,transparent 8px)":"transparent",
                              borderRight:`1px solid ${isToday?"rgba(245,158,11,0.3)":"rgba(26,36,56,0.5)"}`,
                              overflow:"visible",
                            }}>
                              {barsHere.map(({ job, ji, span }) => (
                                <div key={ji} style={{
                                  position:"absolute", top:jobs.length>1?3+ji*22:5, left:2,
                                  width:span*COL_W-4, height:18, borderRadius:2,
                                  background:GANTT_COLORS[(ri*3+ji)%GANTT_COLORS.length],
                                  opacity:0.85, zIndex:2, cursor:"pointer", overflow:"hidden",
                                }}
...
function POStatusPanel() {
  const [filterStatus, setFilterStatus] = useStateTracking("all");
  const [filterETA,    setFilterETA]    = useStateTracking("all");
  const [search,       setSearch]       = useStateTracking("");
  const [sortKey,      setSortKey]      = useStateTracking("poNo");
  const [sortAsc,      setSortAsc]      = useStateTracking(true);
  const [expanded,     setExpanded]     = useStateTracking(new Set());

  const stats = useMemoTracking(() => ({
...
function ProgressTrackingViz({ data }) {
  DAILY_JSON = data.ProgressTracker.productionStatus.daily_data;
  PO_JSON    = data.ProgressTracker.productionStatus.tracking_data;
  ...
  const [mainTab, setMainTab] = useStateTracking("daily");
...
window.ProgressTrackingViz = ProgressTrackingViz;
