...
@pytest.fixture
def validation_orchestrator(shared_source_config):
...
@pytest.fixture
def execution_result(validation_orchestrator):
    """Run validation and get result"""
    return validation_orchestrator.run_validations_and_save_results()
...
class TestIntegration:
    ...
    @pytest.mark.parametrize("config", [
        ...
        {"enable_parallel": True, "max_workers": 1},
        {"enable_parallel": True, "max_workers": 2},
        {"enable_parallel": True, "max_workers": 4},
    ])
    def test_different_configurations(self, shared_source_config, config):
    ...
    def test_parallel_execution(self, shared_source_config):
        """Parallel execution should return valid result"""
        orchestrator = ValidationOrchestrator(
            shared_source_config=shared_source_config,
...
# === HELPER FUNCTIONS ===
def assert_valid_execution_result(result):
    """Helper to assert a valid ExecutionResult"""
    assert result is not None
    ...
    assert result.status in [s.value for s in ExecutionStatus]
    assert result.severity in [s.value for s in PhaseSeverity]
