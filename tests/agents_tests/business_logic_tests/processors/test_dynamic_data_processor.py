...
class TestDynamicDataProcessorInit:
    ...
    def test_init_with_default_schema(self):
        """Test initialization with default productRecords schema"""
        ...
        assert len(processor.required_fields) > 0
        assert 'recordDate' in processor.required_fields
    
    def test_init_with_purchase_orders_schema(self):
        """Test initialization with default purchaseOrders schema"""
    ...
    def test_init_with_custom_schema(self):
        ...
        assert processor.folder_path == '/custom/path'
        assert processor.name_start == 'custom_'
        assert processor.file_extension == '.csv'
        assert processor.sheet_name == 'CustomSheet'
        assert processor.required_fields == ['field1', 'field2']
    
    def test_init_with_empty_database_schema(self):
    ...
    def test_init_with_unknown_data_name(self):
        ...
        with pytest.raises(KeyError):
            processor = DynamicDataProcessor(data_name='unknownDataType')
            # Trying to access any schema attribute will fail
            _ = processor.folder_path
    ...
    def test_schema_deep_copy(self):
        ...
        original_schema['productRecords']['path'] = '/modified/path'
