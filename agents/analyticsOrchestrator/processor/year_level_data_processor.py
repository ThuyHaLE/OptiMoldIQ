...
class YearLevelDataProcessor(ConfigReportMixin):
    ...
    def process_records(self) -> ProcessorResult:
        ...
        self.logger.info("Starting YearLevelDataProcessor ...")
        ...
        try:
            ...
            analysis_timestamp, adjusted_record_year = self._validate_analysis_parameters()
            self.logger.info("Initial year level data processing at {}", adjusted_record_year)
            processor_log_entries.append(f"Initial year level data processing at {adjusted_record_year}")
            ...
            processed_data = {
                    "filteredRecords": po_based_df,
                    "finishedRecords": finished_df,
                    "unfinishedRecords": unfinished_df
                    }
            
            was_adjusted = (adjusted_record_year != self.record_year)

            #analysis_timestamp, adjusted_record_year, processed_data, final_summary
            return ProcessorResult(
                processor_level = ProcessorLevel.YEAR,
                record_year = self.record_year, 
                analysis_date = self.analysis_date,
                adjusted_record_year = adjusted_record_year, 
    ...
    def _validate_analysis_parameters(self) -> Tuple[pd.Timestamp, str]:
            ...
            return analysis_timestamp, adjusted_record_year
