...
class YearLevelDataProcessor:
    ...
    def __init__(self,
        ...
        self.filename_prefix = "year_level"
        self.default_dir = Path(default_dir)
        self.output_dir = self.default_dir / "YearLevelDataProcessor"
    ...
    def data_process(self, 
        ...
        if save_output: 
            ...
            try:
                excel_file_name = f"{timestamp_file}_{self.filename_prefix}_insights_{adjusted_record_year}.xlsx"
            ...
            try:
                analysis_summary_name = f"{timestamp_file}_{self.filename_prefix}_summary_{adjusted_record_year}.txt"
    ...
    def product_record_processing(self):
        ...
        po_based_df = self._filter_data(adjusted_record_year, analysis_timestamp)
        ...
        analysis_summary = YearLevelDataProcessor._log_analysis_summary(
            analysis_timestamp, 
            adjusted_record_year, 
            po_based_df)
        ...
        unfinished_df = YearLevelDataProcessor._analyze_unfinished_pos(combined_df, analysis_timestamp)
    ...
    def _detect_backlog(self,
        ...
        """
        Detect backlog purchase orders for a given year based on ETA and production status.

        A purchase order (PO) is considered **backlog** if:
            - Its ETA (Expected Time of Arrival) is *before* the target record year, **and**
        ...
        year but remain unfinished as of the start of that year.

        Args:
            record_year (str):
                The target analysis year, in format `"YYYY"`.
                Example: `"2024"`.
        ...
        Raises:
            ValueError:
                If `record_year` is not in valid `"YYYY"` format.
            KeyError:
                If required purchase order columns are missing, or the merged DataFrame
                does not contain `'proStatus'`.

        Notes:
            - Backlog detection is performed **at the start of the record_year**.
            - Useful for identifying carryover workload or delayed production items.
        """

        # Validate and parse record_year format
        try:
            # Convert string 'YYYY-MM' to pandas.Period (yearly granularity)
            record_period = pd.Period(record_year, freq="Y")
        except ValueError as e:
            raise ValueError(
                f"Invalid record_year format '{record_year}'. Expected 'YYYY'"
            ) from e

        self.logger.info("Record year (requested): {}", record_period)

        # Determine the cutoff date for backlog detection
        # Cutoff = last day of previous year (before the record_year starts)
        ...
        po_df["itemCodeName"] = YearLevelDataProcessor._create_item_code_name(po_df)

        # Ensure ETA is datetime type for date comparison
        po_df["poETA"] = pd.to_datetime(po_df["poETA"])

        # Filter purchase orders that should have arrived before record_year
        ...
        merged_df = YearLevelDataProcessor._merge_purchase_status(filtered_po, filtered_records)
    ...
    def _filter_data(self,
                    ...
                    analysis_timestamp: str) -> pd.DataFrame:
        """
        Filter purchase orders by record_year and combine with backlog data.

        This function filters purchase orders for a given year (based on their ETA),
        merges them with production status information, and appends backlog orders
        that were expected before this year but remain unfinished.
        It ensures that the analysis includes both *current-year* and *carryover* orders.

        Business Logic:
            1. Select all purchase orders (POs) whose ETA falls within `record_year`.
            2. Merge these with production records available up to `analysis_timestamp`.
            3. Detect backlog orders from previous years (via `_detect_backlog`).
            4. Combine backlog and current-year orders into a unified dataset.

        Args:
            record_year (str):
                Target year in format `"YYYY"`.
                Example: `"2024"`.
            analysis_timestamp (str):
        ...
        Returns:
            pd.DataFrame:
                Combined DataFrame containing:
                    - Purchase orders for the target year
                    - Backlog orders from earlier years
                    - Production status fields
                    - An `'is_backlog'` flag marking backlog orders (`True` / `False`)

        Raises:
            ValueError:
                If record_year format is invalid
                or if no matching data exists for that year.
            KeyError:
        ...
        # Validate and parse record_year
        try:
            record_period = pd.Period(record_year, freq="Y")
        except (ValueError, TypeError) as e:
            raise ValueError(f"Invalid record_year format '{record_year}'. Expected 'YYYY'.") from e
        ...
        purchase_orders_df["itemCodeName"] = YearLevelDataProcessor._create_item_code_name(purchase_orders_df)

        # Verify that the requested year exists in the data
        # Convert ETA → yearly period for filtering
        available_years = purchase_orders_df["poETA"].dt.to_period("Y").dropna().unique()
        available_years_sorted = sorted(available_years)
        self.logger.info("Available years in data: {}", available_years_sorted)

        if record_period not in available_years:
            self.logger.error(
                "Requested year {} not found. Available years: {}",
                record_period,
                available_years_sorted
            )
            raise ValueError(
                f"Requested year {record_period} not found in purchase orders. "
                f"Available years: {available_years_sorted}"
            )

        # Filter POs belonging to the target year
        filtered_purchase_orders = purchase_orders_df[
            purchase_orders_df["poETA"].dt.to_period("Y") == record_period
        ].copy()

        self.logger.info(
            "Filtered {} purchase orders for year: {}",
        ...
        if filtered_product_records.empty:
            ...
            self.logger.error(
                "No historical data available for the target year ({}). "
                "Earliest record date is {}.",
                record_year, filtered_product_records['recordDate'].min().date()
            )
            raise ValueError(
                f"No historical data available for the target year ({record_year}). "
        ...
        # Merge current-year POs with their production status
        purchase_status_df = YearLevelDataProcessor._merge_purchase_status(
            filtered_purchase_orders, filtered_product_records)

        # Mark as non-backlog
        purchase_status_df['is_backlog'] = False

        # Detect backlog POs (unfinished from earlier years)
        backlog_df = self._detect_backlog(record_year, analysis_timestamp, filtered_product_records)
    ...
    def _validate_analysis_parameters(self) -> Tuple[pd.Timestamp, str]:
            ...
            This method ensures that the analysis date (`analysis_date`) and the record year
            (`record_year`) used for manufacturing analysis are logically consistent with the
            available production records (`productRecords_df`).

            Business Rules:
                1. Analysis date looking at a past or current record_year → ✅ OK
                2. Analysis date looking at a *future* record_year → ❌ FAILED
                3. If analysis_date > max(recordDate):
                    → Adjust analysis_date to the latest available recordDate
                    → Adjust record_year accordingly to match that date
                4. If no historical data is available → ❌ FAILED

            Behavior:
                - If `analysis_date` is not provided, the function automatically uses
                the end of the specified `record_year` as the analysis date.
                - Ensures that both `analysis_date` and `record_year` are valid and
                correctly formatted (`YYYY-MM-DD` and `YYYY` respectively).
            ...
            Returns:
                Tuple[pd.Timestamp, str]:
                    ...
                    - adjusted_record_year (str):
                        The corresponding record year (possibly adjusted if analysis date was shifted).

            Raises:
                ValueError:
                    - If no historical production data is available.
                    - If all recordDate values are null.
                    - If record_year or analysis_date formats are invalid.
                    - If analysis_date points to a future record_year.
            ...
            self.logger.info(
                ...
                self.productRecords_df['recordDate'].min().year,
                max_record_date.year
            )

            # Parse and validate record_year
            try:
                # Convert record_year string ('YYYY') → pandas.Period
                requested_period = pd.Period(self.record_year, freq="Y")
            except Exception as e:
                self.logger.error(
                    "Invalid record_year format '{}'. Expected 'YYYY'. Error: {}",
                    self.record_year, e
                )
                raise ValueError(
                    f"Invalid record_year format '{self.record_year}'. Expected 'YYYY'. Error: {e}"
                )

            # Determine analysis date
            if self.analysis_date is None:
                # If not specified → use end of the record year as default
                analysis_timestamp = requested_period.to_timestamp(how="end").normalize()
                self.logger.info(
                    "No analysis_date provided. Using the end of record_year as analysis date: {}",
            ...
            # Business rule: no future-year analysis
            analysis_period = analysis_timestamp.to_period("Y")

            # If analysis_date points to a *future* year → reject
            if analysis_period < requested_period:
                self.logger.error(
                    "Invalid: Analysis date ({}) is before record_year ({}). "
                    "Cannot analyze a future year from a past date!",
                    analysis_timestamp.date(), requested_period
                )
                raise ValueError(
                    f"Invalid: Analysis date ({analysis_timestamp.date()}) is before record_year "
                    f"({requested_period}). Cannot analyze a future year from a past date!"
                )

            # Adjust analysis date if it exceeds available data
            original_analysis_date = analysis_timestamp
            adjusted_record_year = self.record_year  # Track if we adjust the year
            ...
            if analysis_timestamp > max_record_date:
                ...
                # Adjust record_year accordingly
                adjusted_period = analysis_timestamp.to_period("Y")
                adjusted_record_year = adjusted_period.strftime("%Y")

                self.logger.warning(
                    "Adjusting analysis date to max record date ({}) and record year from {} to {}",
                    analysis_timestamp.date(), requested_period, adjusted_period
                )

            # Log summary of validation results
            summary_text = YearLevelDataProcessor._log_validation_summary(
                self.record_year,
            ...
            return analysis_timestamp, adjusted_record_year, summary_text
    ...
    def _process_pro_status(product_records_df: pd.DataFrame) -> pd.DataFrame:
        ...
        pro_status = (
            ...
            .agg(
                ...
                itemNGQuantity=("itemNGQuantity", "sum"),     # Total NG quantity
    ...
    def _merge_purchase_status(purchase_orders, product_records):
        ...
        pro_status = YearLevelDataProcessor._process_pro_status(product_records)
    ...
    def _analyze_unfinished_pos(unfinished_df: pd.DataFrame,
                                analysis_timestamp) -> pd.DataFrame:
    ...
    @staticmethod
    def _log_analysis_summary(analysis_timestamp,
                              adjusted_record_year,
                              df):
        
        # Validate df columns
        cols = ['poETA', 'itemQuantity', 'itemGoodQuantity', 'is_backlog', 'poStatus']
        ...
        lines.append("=" * 60)
        lines.append(f"Record year (requested): {adjusted_record_year}")
        lines.append(f"Analysis date (validated): {analysis_timestamp.date()}")
        ...
        for m_info, m_df in df.groupby(df['poETA'].dt.strftime('%Y-%m')):
            ...
            # --- PO status summary ---
            lines.extend(get_po_status_summary(m_df))
