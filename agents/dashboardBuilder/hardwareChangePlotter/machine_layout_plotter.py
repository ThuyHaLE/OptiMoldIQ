...
from agents.dashboardBuilder.visualize_data.machine_level.individual_machine_layout_change_plotter import individual_machine_layout_change_plotter
from agents.dashboardBuilder.visualize_data.machine_level.machine_layout_change_plotter import machine_layout_change_plotter
...
@validate_init_dataframes({
    "machine_layout_hist_change_df": REQUIRED_COLUMNS, 
})

class MachineLayoutPlotter:
    """
    Plotter for machine-level dashboard with visualization and reporting.
    """
    
    def __init__(self, 
                 machine_layout_hist_change_df: pd.DataFrame, 
                 default_dir: str = "agents/shared_db/DashboardBuilder/HardwareChangePlotter/MachineLayoutPlotter",
                 visualization_config_path: str = None,
                 enable_parallel: bool = True,
                 max_workers: int = None):
        
        self.logger = logger.bind(class_="MachineLayoutPlotter")
        ...
        self.visualization_config_path = (
            ...
            or "agents/dashboardBuilder/visualize_data/machine_level/visualization_config.json"
        )
        
        self.machine_layout_hist_change_df = self._prepare_data(machine_layout_hist_change_df)
    ...
    def _prepare_data(self, machine_layout_hist_change_df) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Prepare dataframes for visualization.
        """

        # Validate dataframe
        validate_dataframe(machine_layout_hist_change_df, ['machineCode', 'machineName'])

        # Prepare data - Convert into long format
        layout_change_df = machine_layout_hist_change_df.copy()
        date_columns = [col for col in layout_change_df.columns if col not in ['machineCode', 'machineName']]
        melted_df = layout_change_df.melt(id_vars=['machineCode', 'machineName'],
                                                    value_vars=date_columns,
                                                    var_name='changedDate',
                                                    value_name='machineNo')
        melted_df = melted_df.dropna(subset=['machineNo'])
        melted_df['changedDate'] = pd.to_datetime(melted_df['changedDate'])
        melted_df['machineNo_numeric'] = melted_df['machineNo'].str.replace('NO.', '').astype(int)
    ...
    def _prepare_plot_tasks(self, timestamp_file: str, newest_dir: Path) -> list:
        ...
        plots_args = plots_args = [
            (self.machine_layout_hist_change_df,
