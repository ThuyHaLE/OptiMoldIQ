...
class HybridSuggestOptimizer(ConfigReportMixin):
    ...
    def process(self) -> OptimizationResult:
        ...
        try:
            optimization_log_lines.append("Starting Phase 1: ItemMoldCapacityOptimizer...")
            (estimated_capacity_invalid_molds, mold_estimated_capacity_df,
            estimation_log) = self._estimate_mold_capacities()
            optimization_log_lines.append("⤷ ItemMoldCapacityOptimizer results:")
    ...
    def _estimate_mold_capacities(self) -> Tuple[List[str], pd.DataFrame, str]:
        """Estimate mold capacities using ItemMoldCapacityOptimizer."""   
        try:
            self.logger.debug("Initializing ItemMoldCapacityOptimizer...")
            optimizer = ItemMoldCapacityOptimizer(
                shared_source_config=self.config.shared_source_config,
                efficiency=self.config.efficiency,
                loss=self.config.loss
            )
            self.logger.debug("ItemMoldCapacityOptimizer initialized successfully")
            return optimizer.process()
        except Exception as e:
            self.logger.error("Failed to initialize/run ItemMoldCapacityOptimizer: {}", str(e))
            raise
