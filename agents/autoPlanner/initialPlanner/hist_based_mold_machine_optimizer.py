...
class HistBasedMoldMachineOptimizer:
    ...
    @staticmethod
    def create_binary_priority_matrix(priority_matrix: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug("Converting priority matrix of shape {} to binary", priority_matrix.shape)

        return (priority_matrix == 1).astype(np.int8)  # Use int8 for memory efficiency

    def create_leadtime_matrix(self,
                               binary_priority: pd.DataFrame,
                               lead_time_df: pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            ...
            lead_time_df: DataFrame containing moldNo and moldLeadTime columns

        Returns:
            Lead time matrix with adjusted values
        """

        self.logger.debug("Creating lead time matrix")

        # Create efficient mapping using dictionary for O(1) lookup
        leadtime_mapping = dict(zip(lead_time_df['moldNo'], lead_time_df['moldLeadTime']))

        # Use vectorized operations for better performance
        leadtime_series = pd.Series([leadtime_mapping.get(idx, 0) for idx in binary_priority.index],
                                    index=binary_priority.index, dtype='int32')

        # Multiply each row by corresponding lead time using broadcasting
        leadtime_matrix = binary_priority.mul(leadtime_series, axis=0).astype('int32')

        self.logger.debug("Lead time matrix created with {} non-zero entries",
                     (leadtime_matrix > 0).sum().sum())

        return leadtime_matrix

    @staticmethod
    def calculate_machine_statistics(leadtime_matrix: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:

        """
        Calculate statistics for each machine efficiently without copying the entire matrix.

        Args:
            leadtime_matrix: Matrix containing lead times
        ...
        # Use vectorized operations for better performance
        suitable_count = (leadtime_matrix > 0).sum(axis=0)
        total_leadtime = leadtime_matrix.sum(axis=0)
    ...
    def find_valid_pairs(self,
                         leadtime_matrix: pd.DataFrame,
                         leadtime_mapping: Dict[str, int],
        ...
        Args:
            leadtime_matrix: Current lead time matrix
            leadtime_mapping: Pre-computed mapping of mold to lead time
        ...
        suitable_count, _ = self.calculate_machine_statistics(leadtime_matrix)
        ...
        for machine in target_machines:
            ...
            compatible_molds = leadtime_matrix[leadtime_matrix[machine] > 0].index

            for mold in compatible_molds:
                leadtime = leadtime_mapping.get(mold)
    ...
    def assign_molds_to_machines(self,
                                 ...
                                 leadtime_matrix: pd.DataFrame) -> Tuple[List[Tuple[str, str, int]],
        ...
        Args:
            valid_pairs: List of valid assignments
            results_matrix: Results matrix to update
            leadtime_matrix: Lead time matrix to modify

        Returns:
            Tuple of (valid_pairs, updated_results, updated_leadtime_matrix)
        """

        if not valid_pairs:
            return valid_pairs, results_matrix, leadtime_matrix

        # Batch update results matrix
        for mold, machine, leadtime in valid_pairs:
            if mold in results_matrix.index and machine in results_matrix.columns:
                results_matrix.loc[mold, machine] = leadtime

        # Efficiently remove assigned molds using vectorized operation
        assigned_molds = [mold for mold, _, _ in valid_pairs]
        if assigned_molds:
            leadtime_matrix.loc[assigned_molds] = 0

        self.stats['assignments_made'] += len(valid_pairs)
        self.logger.debug("Assigned {} molds to machines", len(valid_pairs))

        return valid_pairs, results_matrix, leadtime_matrix

    def optimize_mold_machine_matching(self, priority_matrix: pd.DataFrame,
                                       lead_time_df: pd.DataFrame) -> Tuple[pd.DataFrame,
                                                                            List[str], List[str],
                                                                            pd.DataFrame]:
        ...
        Args:
            priority_matrix: Initial priority matrix
            lead_time_df: DataFrame with lead time information

        Returns:
            Tuple of (results_matrix, assigned_molds, unassigned_molds, final_leadtime_matrix)
        """
        ...
        binary_priority = self.create_binary_priority_matrix(priority_matrix)

        # Step 2: Create lead time matrix
        leadtime_matrix = self.create_leadtime_matrix(binary_priority, lead_time_df)

        # Step 3: Initialize results matrix with optimal dtype
        results_matrix = pd.DataFrame(
            data=0,
        ...
        all_assigned_pairs = []
        max_iterations = min(len(priority_matrix) * 2, 1000)  # Reasonable limit
        ...
        while self.stats['iterations'] < max_iterations:
            ...
            valid_pairs = self.find_valid_pairs(leadtime_matrix, leadtime_mapping, target_suitable_count=1)
            ...
            valid_pairs, results_matrix, leadtime_matrix = self.assign_molds_to_machines(
                valid_pairs, results_matrix, leadtime_matrix
            )
        ...
        remaining_molds = leadtime_matrix[leadtime_matrix.sum(axis=1) > 0].index.tolist()
        ...
        return results_matrix, assigned_molds, remaining_molds, leadtime_matrix

    def find_unique_matches(self,
                            lead_time_df: pd.DataFrame,
                            priority_matrix: pd.DataFrame) -> Tuple[List[str], pd.DataFrame, pd.DataFrame]:
        ...
        valid_mold_nos = set(lead_time_df['moldNo'])
        filtered_priority_matrix = priority_matrix[priority_matrix.index.isin(valid_mold_nos)].copy()
        ...
        for mold_no in unique_molds:
            ...
            leadtime = leadtime_mapping.get(mold_no)
            if leadtime is not None:
                unique_match_list.append(mold_no)

        # Split matrices
        unique_match_priority_matrix = filtered_priority_matrix.loc[unique_match_list] if unique_match_list else pd.DataFrame()
        non_unique_match_priority_matrix = filtered_priority_matrix.drop(index=unique_match_list)

        self.stats['unique_matches'] = len(unique_match_list)
        self.logger.info("Found {} unique matches", len(unique_match_list))

        return unique_match_list, unique_match_priority_matrix, non_unique_match_priority_matrix
    ...
    def constraint_based_optimized(self,
        ...
        Args:
            lead_time_df (pd.DataFrame): DataFrame containing lead time info (moldNo, moldLeadTime)
            priority_matrix (pd.DataFrame): Mold-machine compatibility matrix
        ...
        non_unique_match_priority_matrix) = self.find_unique_matches(lead_time_df, priority_matrix)
        ...
        round_one_results = results[~(results == 0).all(axis=1)]

        return round_one_results, non_unique_match_priority_matrix, unique_match_priority_matrix, round_one_assigned_molds, round_one_unassigned_molds
    ...
    @staticmethod
    def _create_candidate_lead_time_matrix(priority_matrix: pd.DataFrame,
                                           unassigned_molds: List,
                                           lead_times: pd.DataFrame):

        """Create candidate lead time matrix for unassigned molds."""

        updated_priority_matrix = priority_matrix[priority_matrix.index.isin(unassigned_molds)].copy()

        binary_priority = (updated_priority_matrix > 0).astype(int)

        best_leadtime_map = lead_times.set_index('moldNo')['moldLeadTime']
    ...
    def load_balance_based_optimized(self,
        ...
        Args:
            unassigned: List of mold numbers that need to be assigned.
            lead_times: DataFrame containing lead time data between molds and machines.
            ...
            results: DataFrame containing prior assignment results.
        ...
        total_machine_load_df = HistBasedMoldMachineOptimizer._update_total_machine_load(producing_data,
                                                                                         results,
                                                                                         machine_info_df)

        # Step 2: Create lead time matrix for unassigned molds and candidate machines
        candidate_lead_time_matrix = HistBasedMoldMachineOptimizer._create_candidate_lead_time_matrix(priority_matrix,
                                                                                                      unassigned,
                                                                                                      lead_times)
        ...
        self.logger.info("Max load threshold: {} days", max_load_threshold)
        ...
        for moldNo in leadtime_df.index:
            ...
            for machine in sorted_machines:
                ...
                if projected_load <= max_load_threshold:
                    ...
                    self.logger.debug("✓ Selected machine {} - {} days (projected load: {} ≤ {})",
                                      selected_machine, selected_load, projected_load, max_load_threshold)
                    break
                else:
                    self.logger.debug("✗ Skipped machine {} (projected load: {} > {})",
                                      machine, projected_load, max_load_threshold)
            ...
            if selected_machine is None:
                ...
                self.logger.debug("⚠ No machine ≤ {} days, choosing best option: {} - {} days",
                             max_load_threshold, selected_machine, selected_load)
    ...
    def run_optimization(self,
                         ...
                         producing_data: pd.DataFrame,
                         machine_info_df: pd.DataFrame,
                         max_load_threshold: int = 30) -> Tuple[pd.DataFrame, List[str], List[str]]:

        """
        Main wrapper function to run the complete optimization process.

        Args:
            priority_matrix: Input priority matrix
            lead_time_df: DataFrame with lead time information
        ...
        (round_one_results,
         ...
         round_one_unassigned_molds) = self.constraint_based_optimized(lead_time_df, priority_matrix)
        ...
        if len(round_one_unassigned_molds) == 0:
            ...
            round_one_results.index.name = 'moldNo'
            return round_one_results, round_one_assigned_molds, round_one_unassigned_molds
        else:
            ...
            (round_two_result,
             ...
             round_two_unassigned_molds) = self.load_balance_based_optimized(
            ...
            final_results = pd.concat([round_one_results, round_two_result], axis=0)
            final_results.index.name = 'moldNo'
            ...
            return final_results, all_assigned_molds, round_two_unassigned_molds