...
from agents.autoPlanner.tools.machine_assignment import (
    create_binary_priority_matrix, calculate_machine_statistics,
    update_total_machine_load, create_candidate_lead_time_matrix)

class HistoryBasedAssigner(ConfigReportMixin): # HistBasedMoldMachineOptimizer

    """
    Assign mold-machine based on optimized algorithm 
    ...
    mold_lead_times_df: : pd.DataFrame
        DataFrame with lead time information
    producing_data: : pd.DataFrame
        Current production data
    machine_info_df: : pd.DataFrame
        Machine information DataFrame
    max_load_threshold: : int, default=30
        Maximum load threshold for round 2
    ...
    def __init__(self,
        ...
        self.logger = logger.bind(class_="HistoryBasedAssigner")
    ...
    def run_assign(self) -> AssignerResult:
        ...
        self.logger.info("Starting HistoryBasedAssigner ...")
        
        self.logger.info("Starting CompatibilityBasedAssigner ...")
        self.stats = AssignerStats()
        ...
        try:
            ...
            round_one_success_rate = (len(round_one_assigned_molds) / 
                                      round_one_total_molds * 100) if round_one_total_molds > 0 else 0
            constraint_based_log = log_constraint_based_optimization(
                stats=self.stats,
                total_molds=round_one_total_molds,  
                assigned_molds=round_one_assigned_molds,  
                unassigned_molds=round_one_unassigned_molds,
                success_rate=round_one_success_rate)
            optimization_log_lines.append(constraint_based_log)
            ...
            if len(round_one_unassigned_molds) == 0:
                ...
                round_two_start_time = datetime.now()
                ...
                round_two_total_time = (datetime.now() - round_two_start_time).total_seconds()

                # Log round 2 results
                round_two_total_molds = len(round_two_assigned_molds) + len(round_two_unassigned_molds)
                round_two_success_rate = (len(round_two_assigned_molds) / 
                                          round_two_total_molds * 100) if round_two_total_molds > 0 else 0
                balance_based_log = log_load_balance_based_optimization(
                    assigned_molds=round_two_assigned_molds,
                    unassigned_molds=round_two_unassigned_molds,    
                    total_molds=round_two_total_molds,
                    success_rate=round_two_success_rate,
                    total_time=round_two_total_time
                )
                optimization_log_lines.append(balance_based_log)
    ...
    def constraint_based_optimized(self) -> Tuple[pd.DataFrame,
                                            pd.DataFrame,
                                            pd.DataFrame,
                                            List[str],
                                            List[str]]:
        ...
        # Phase 2: Perform greedy assignment for remaining molds, prioritizing machines with few compatible options
        ...
        (results, 
         ...
         final_matrix) = self.optimize_mold_machine_matching(non_unique_match_priority_matrix, 
        ...
        if not unique_match_priority_matrix.empty:
            ...
            unique_match_leadtime_matrix = self.create_leadtime_matrix(
                create_binary_priority_matrix(unique_match_priority_matrix),
                self.mold_lead_times_df
            )
    ...
    def find_unique_matches(self,
        ...
        filtered_priority_matrix = mold_machine_priority_matrix[
            mold_machine_priority_matrix.index.isin(valid_mold_nos)].copy()
        ...
        for mold_no in unique_molds:
            ...
            compatible_machine = filtered_priority_matrix.loc[
                mold_no][filtered_priority_matrix.loc[mold_no] != 0].index[0]
            leadtime = mold_leadtime_mapping.get(mold_no)
            if leadtime is not None:
                unique_matched_list.append(mold_no)

        # Split matrices
        unique_matched_priority_matrix = filtered_priority_matrix.loc[
            unique_matched_list] if unique_matched_list else pd.DataFrame()
    ...
    def optimize_mold_machine_matching(self, 
        ...
        binary_priority_matrix = create_binary_priority_matrix(mold_machine_priority_matrix)
    ...
    def find_valid_pairs(self,
        ...
        suitable_count, _ = calculate_machine_statistics(mold_leadtime_matrix)
        self.logger.debug("Calculated statistics for {} machines", len(suitable_count))
    ...
    def load_balance_based_optimized(self,
        ...
        total_machine_load_df = update_total_machine_load(self.producing_data,
                                                          round_one_assigned_matrix,
                                                          self.machine_info_df)

        # Step 2: Create lead time matrix for unassigned molds and candidate machines
        candidate_lead_time_matrix = create_candidate_lead_time_matrix(self.mold_machine_priority_matrix,
                                                                       round_one_unassigned_molds,
                                                                       self.mold_lead_times_df)
