# YearLevelDataPlotter output overview

## Excel Report: 
- *Filename*: `{timestamp}_extracted_records_{year}.xlsx`
- Including 5 sheets (`finished_df`, `unfinished_df`, `short_unfinished_df`, `all_progress_df`, `filtered_df`) generated by `YearLevelDataProcessor` and `YearLevelDataPlotter`
...
- *Summary of Sheet Relationships*:
...
3 sheets (`short_unfinished_df`, `all_progress_df`, `filtered_df`) generated by `YearLevelPlotter` ([→ see detail](https://github.com/ThuyHaLE/OptiMoldIQ/blob/main/docs/documentations/dashboardBuilder/OptiMoldIQ_yearLevelDataPlotter_overview.md)) 

#### 3. Sheet 3 — short_unfinished_df:
...
## TXT Report: 
- *Filename*: `{timestamp}_final_summary_{year}.txt`
...
- *Generated by*: `field_based_month_view_dashboard_plotter()` with different configurations
- *Purpose*:
